...
{block name=title}{Org->name}{/block}
{block name=css}
	<style>
		#sla-daily-graph, #sla-distribution-graph { height: 320px; }
...
{block name=content}
	...
	<div class="row">
		<div class="col-sm-12">
			<h4 data-bind="text: type">Organization SLA Report</h4>
	...
	<div class="row">
		<div class="col-sm-12">
			...
			<table id="measurements" class="table table-condensed">
				<thead><tr><th>Date</th><!-- ko if: slaIsTrim() == false --><th>Avg</th><!-- /ko --><!-- ko if: slaIsTrim() --><th>5% Trim Avg</th><!-- /ko --></thead>
				<tbody data-bind="foreach: measurements">
				<tr data-bind="attr: { class: className }"><td data-bind="text: date"></td><!-- ko if: $parent.slaIsTrim() == false --><td data-bind="text: avg"></td><!-- /ko --><!-- ko if: $parent.slaIsTrim() --><td data-bind="text: trim"></td><!-- /ko --></tr>
				</tbody>
			</table>
		</div>
	</div>
...
{block name=script}
	...
	<script src="{$DIR_VENDOR}knockout/knockout-3.3.0.js"></script>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			function updateView() {
				...
				$("#content").block({ message: "<h4><img src=\"{$DIR_IMG}ajax-loader-bar-black.gif\"> Requesting measurements...</h4>" });
			...
			function updateMeasurements(src) {
				...
				var measurementLabel = "Average";
				var measurementData = avg;
				if (viewModel.slaIsTrim()) {
					measurementLabel = viewModel.slaIsTrim() + "% Trimmed Avg";
					measurementData = trm;
				}

				$.plot("#sla-daily-graph", [{ "label": measurementLabel, data: measurementData }], {
					xaxis: {
						mode: "time",
						timeformat: "%m/%d/%Y",
						timezone: "browser",
