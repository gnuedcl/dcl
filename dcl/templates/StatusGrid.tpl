...
{block name=content}
<p>
{if $PERM_ADD}<a class="btn btn-success" href="{dcl_url_action controller=Status action=Create}">{$smarty.const.STR_CMMN_NEW}</a>{/if}
...
<script type="text/javascript">
    $(document).ready(function() {
		$("#grid").jqGrid({
		   	url: '{$URL_MAIN_PHP}?menuAction=StatusService.GetData',
			datatype: "json",
		   	colNames:[
	...
	function formatOptions(value, options, row) {
		...
		{if $PERM_EDIT}retVal.push('<a class="btn btn-primary btn-xs" href="{dcl_url_action controller=Status action=Edit params="id="}' + row[0] + '" title="{$smarty.const.STR_CMMN_EDIT}"><span class="glyphicon glyphicon-pencil"></span></a>');{/if}
		{if $PERM_DELETE}retVal.push('<a class="btn btn-danger btn-xs" href="{dcl_url_action controller=Status action=Delete params="id="}' + row[0] + '" title="{$smarty.const.STR_CMMN_DELETE}"><span class="glyphicon glyphicon-trash"></span></a>');{/if}
