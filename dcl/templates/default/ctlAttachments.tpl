...
{/if}
			<tr>
				<td><a href="{$VAL_FORMACTION}?menuAction=WorkOrder.DownloadAttachment&jcn={$VAL_JCN}&seq={$VAL_SEQ}&filename={$VAL_ATTACHMENTS[file].filename|escape:"rawurl"}">{$VAL_ATTACHMENTS[file].filename}</a></td>
				<td class="numeric">{$VAL_ATTACHMENTS[file].filesize}</td>
				<td>{$VAL_ATTACHMENTS[file].filedate}</td>
				{if $PERM_REMOVEFILE}<td><a href="{$VAL_FORMACTION}?menuAction=WorkOrder.DeleteAttachment&jcn={$VAL_JCN}&seq={$VAL_SEQ}&filename={$VAL_ATTACHMENTS[file].filename|escape:"rawurl"}">{$smarty.const.STR_CMMN_DELETE}</a></td>{/if}
			</tr>
