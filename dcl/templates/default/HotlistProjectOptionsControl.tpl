...
<li {if $ctlHotlistProjectOptions_isfirst == "true"}{assign var="ctlHotlistProjectOptions_isfirst" value="false"}class="first"{/if}><a href="{$URL_MAIN_PHP}?menuAction=Hotlist.Edit&hotlist_id={$VAL_HOTLISTID}">{$smarty.const.STR_CMMN_EDIT}</a></li>
<li {if $ctlHotlistProjectOptions_isfirst == "true"}{assign var="ctlHotlistProjectOptions_isfirst" value="false"}class="first"{/if}><a href="{$URL_MAIN_PHP}?menuAction=Hotlist.Delete&hotlist_id={$VAL_HOTLISTID}">{$smarty.const.STR_CMMN_DELETE}</a></li>
{/strip}
</ul></th></tr>