<p>
{if $PERM_ADD}<a class="positive button" href="{dcl_url_action controller=Priority action=Create}">{$smarty.const.STR_CMMN_NEW}</a>{/if}
...
<script type="text/javascript">{literal}
    $(document).ready(function() {
		$("#grid").jqGrid({
		   	url: '{/literal}{$URL_MAIN_PHP}?menuAction=PriorityService.GetData{literal}',
			datatype: "json",
		   	colNames:[{/literal}
		    ...
		    viewrecords: true,
			hidegrid: false,
		    caption: "{/literal}{$smarty.const.STR_PRIO_TABLETITLE|escape:"javascript"}{literal}"
		})
	...
	function formatOptions(value, options, row) {
		var retVal = [];
		{/literal}
		{if $PERM_EDIT}retVal.push('<a class="button" href="{dcl_url_action controller=Priority action=Edit params="id="}' + row[0] + '">{$smarty.const.STR_CMMN_EDIT}</a>');{/if}
		{if $PERM_DELETE}retVal.push('<a class="negative button" href="{dcl_url_action controller=Priority action=Delete params="id="}' + row[0] + '">{$smarty.const.STR_CMMN_DELETE}</a>');{/if}
		{literal}
		return retVal.join(' ');
	}
{/literal}</script>