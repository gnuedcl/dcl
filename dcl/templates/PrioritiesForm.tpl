...
<form class="form-horizontal" method="post" action="{$URL_MAIN_PHP}">
	...
	<fieldset>
		...
		{dcl_form_control id=active controlsize=10 label=$smarty.const.STR_PRIO_ACTIVE required=true}
		{$CMB_ACTIVE}
		{/dcl_form_control}
		{dcl_form_control id=short controlsize=4 label=$smarty.const.STR_PRIO_SHORT required=true}
		{dcl_input_text id=short maxlength=10 value=$VAL_SHORT}
		{/dcl_form_control}
		{dcl_form_control id=name controlsize=4 label=$smarty.const.STR_PRIO_NAME required=true}
		{dcl_input_text id=name maxlength=20 value=$VAL_NAME}
		{/dcl_form_control}
		{dcl_form_control id=weight controlsize=4 label=$smarty.const.STR_PRIO_WEIGHT required=true}
	...
	<fieldset>
		<div class="row">
			<div class="col-sm-offset-2">
				...
				<input class="btn btn-link" type="button" onclick="location.href = '{$URL_MAIN_PHP}?menuAction=Priority.Index';" value="{$smarty.const.STR_CMMN_CANCEL}">
			</div>
		</div>
	</fieldset>
</form>
