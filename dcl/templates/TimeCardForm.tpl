...
{block name=title}{if $IS_BATCH}{$smarty.const.STR_TC_BATCHUPDATE|escape}{elseif $IS_EDIT}{$smarty.const.STR_TC_EDIT|escape}{else}Add New Time Card to [{$VAL_JCN}-{$VAL_SEQ}]{/if}{/block}
{block name=css}
...
<form class="form-horizontal" name="NewAction" method="post" action="{$URL_MAIN_PHP}" enctype="multipart/form-data">
	...
	<fieldset>
		<legend>{if $IS_BATCH}{$smarty.const.STR_TC_BATCHUPDATE|escape}{elseif $IS_EDIT}{$smarty.const.STR_TC_EDIT|escape}{else}Add New Time Card to [{$VAL_JCN}-{$VAL_SEQ}]{/if}</legend>
...
{if $IS_BATCH}{include file="Table.tpl"}{/if}
{/block}
{block name=script}
{dcl_validator_init}
