...
class EnvironmentService
{
	public function GetData()
	{
		...
		$validColumns = array('environment_id', 'active', 'environment_name');
		...
		$nameFilter = @$_REQUEST['environment_name'];

		$model = new EnvironmentModel();
		$queryHelper = new EnvironmentSqlQueryHelper();
		$queryHelper->AddDef('columns', '', array('environment_id', 'active', 'environment_name'));

		if ($idFilter !== null)
			$queryHelper->AddDef('filter', 'environment_id', $idFilter);

		if (isset($activeFilter))
			$queryHelper->AddDef('filter', 'active', $model->Quote($activeFilter));

		if (isset($nameFilter))
			$queryHelper->AddDef('filterlike', 'environment_name', $nameFilter);
