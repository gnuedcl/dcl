...
function smarty_function_dcl_get_entity_hotlist($params, &$smarty)
{
	...
	foreach ($aHotlists as $item)
	{
		...
		if ($useLinks)
			echo '<a class="dcl-hotlist" href="' . DCL_WWW_ROOT . 'main.php?menuAction=htmlHotlists.browse&tag=' . urlencode($item['hotlist']) . '">' . htmlspecialchars($item['hotlist'], ENT_QUOTES) . ' #' . $item['priority'] . '</a>';
		else
