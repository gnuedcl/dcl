  ...
  \**************************************************************************/
	...
	class SchemaManagerPgsql
	{
		...
		public function AddColumn($oProc, &$aTables, $sTableName, $sColumnName, &$aColumnDef)
		{
			...
			if (($bRetVal = !!($oProc->m_odb->query($query))) && isset($default))
			{
				$sDefault = $this->TranslateDefault($default, $aColumnDef['type']);
				$query = "ALTER TABLE $sTableName ALTER COLUMN $sColumnName SET $sDefault;\n";
				
				$query .= "UPDATE $sTableName SET $sColumnName=";
				...
				$query .= ";\n";
				$bRetVal = !!($oProc->m_odb->query($query));

				if ($bRetVal && $notnull)
				{
					$oProc->m_odb->query('SELECT version()');
					$iMajor = 7;
					$iMinor = 1;
					$iRev = 0;
					$bCanAlter = false;
					if ($oProc->m_odb->next_record())
					{
						$sVersion = $oProc->m_odb->f(0);
						$iVersionPos = mb_strpos($sVersion, ' ') + 1;
						$sVersion = mb_substr($sVersion, $iVersionPos, mb_strpos($sVersion, ' ', $iVersionPos) - $iVersionPos + 1);
						...
						$bCanAlter = ($iMajor > 7 || ($iMajor == 7 && $iMinor >= 3));
					}
					
					$oProc->m_odb->FreeResult();
					...
					{
						$this->AlterColumn($oProc, $aTables, $sTableName, $sColumnName, $aColumnDef);
					}
				}
			}
			return $bRetVal;
