...
function removeColumn(f) {
	...
	var sValue = f.elements["columnList"].options[f.elements["columnList"].selectedIndex].value;
	if (sValue == "jcn") {
		alert("WO# must appear in the column list!");
		return;
	}

	if (sValue == "seq") {
		alert("Seq must appear in the column list!");
		return;
	}

	// Check to see if it's in the order list and remove it from there, too
	var c = f.elements["orderList"];
	for (var i = 0; i < c.options.length; i++)
	{
		if (c.options[i].value == sValue)
		{
			c.options[i] = null;
			break;
		}
