<?php
/*
 * $Id: class.htmlContactTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:47 mdean Exp $
 *
...
class htmlContactTypeView extends htmlView
{
	function htmlContactTypeView()
	{
		...
		$this->sSortAction = 'htmlContactTypeView.execurl';
	}
	...
	function _SetActionFormOptions()
	{
		...
		if ($g_oSec->HasPerm(DCL_ENTITY_CONTACTTYPE, DCL_PERM_ADD))
			$aLinks[STR_CMMN_NEW] = menuLink('', 'menuAction=htmlContactType.add');
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('contact_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_CONTACTTYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlContactType.modify&contact_type_id=' . $id), false);

		if ($g_oSec->HasPerm(DCL_ENTITY_CONTACTTYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlContactType.delete&contact_type_id=' . $id), true);
