<?php
/*
 * $Id: class.htmlAddrTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:51 mdean Exp $
 *
...
class htmlAddrTypeView extends htmlView
{
	function htmlAddrTypeView()
	{
		...
		$this->sSortAction = 'htmlAddrTypeView.execurl';
	}
	...
	function _SetActionFormOptions()
	{
		...
		if ($g_oSec->HasPerm(DCL_ENTITY_ADDRTYPE, DCL_PERM_ADD))
			$aLinks[STR_CMMN_NEW] = menuLink('', 'menuAction=htmlAddrType.add');
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('addr_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_ADDRTYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlAddrType.modify&addr_type_id=' . $id), false);
			
		if ($g_oSec->HasPerm(DCL_ENTITY_ADDRTYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlAddrType.delete&addr_type_id=' . $id), true);
