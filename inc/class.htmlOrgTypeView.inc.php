<?php
/*
 * $Id: class.htmlOrgTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:42 mdean Exp $
 *
...
class htmlOrgTypeView extends htmlView
{
	function htmlOrgTypeView()
	{
		...
		$this->sSortAction = 'htmlOrgTypeView.execurl';
	}
	...
	function _SetActionFormOptions()
	{
		...
		if ($g_oSec->HasPerm(DCL_ENTITY_ORGTYPE, DCL_PERM_ADD))
			$aLinks[STR_CMMN_NEW] = menuLink('', 'menuAction=htmlOrgType.add');
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('org_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_ORGTYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlOrgType.modify&org_type_id=' . $id), false);

		if ($g_oSec->HasPerm(DCL_ENTITY_ORGTYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlOrgType.delete&org_type_id=' . $id), true);
