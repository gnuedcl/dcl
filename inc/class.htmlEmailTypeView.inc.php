<?php
/*
 * $Id: class.htmlEmailTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:49 mdean Exp $
 *
...
class htmlEmailTypeView extends htmlView
{
	function htmlEmailTypeView()
	{
		...
		$this->sSortAction = 'htmlEmailTypeView.execurl';
	}
	...
	function _SetActionFormOptions()
	{
		...
		if ($g_oSec->HasPerm(DCL_ENTITY_EMAILTYPE, DCL_PERM_ADD))
			$aLinks[STR_CMMN_NEW] = menuLink('', 'menuAction=htmlEmailType.add');
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('email_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_EMAILTYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlEmailType.modify&email_type_id=' . $id), false);
		
		if ($g_oSec->HasPerm(DCL_ENTITY_EMAILTYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlEmailType.delete&email_type_id=' . $id), true);
