<?php
/*
 * $Id: class.htmlPhoneTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:42 mdean Exp $
 *
...
class htmlPhoneTypeView extends htmlView
{
	function htmlPhoneTypeView()
	{
		...
		$this->sSortAction = 'htmlPhoneTypeView.execurl';
	}
	...
	function _SetActionFormOptions()
	{
		...
		if ($g_oSec->HasPerm(DCL_ENTITY_PHONETYPE, DCL_PERM_ADD))
			$aLinks[STR_CMMN_NEW] = menuLink('', 'menuAction=htmlPhoneType.add');
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('phone_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_PHONETYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlPhoneType.modify&phone_type_id=' . $id), false);

		if ($g_oSec->HasPerm(DCL_ENTITY_PHONETYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlPhoneType.delete&phone_type_id=' . $id), true);
