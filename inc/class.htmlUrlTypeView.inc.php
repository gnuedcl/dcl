<?php
/*
 * $Id: class.htmlUrlTypeView.inc.php,v 1.1.1.1 2006/11/27 05:30:43 mdean Exp $
 *
...
class htmlUrlTypeView extends htmlView
{
	function htmlUrlTypeView()
	{
		...
		$this->sSortAction = 'htmlUrlTypeView.execurl';
	}
	...
	function _DisplayOptions()
	{
		...
		$id = $this->oDB->f('url_type_id');

		if ($g_oSec->HasPerm(DCL_ENTITY_URLTYPE, DCL_PERM_MODIFY))
			$this->_AddDisplayOption(STR_CMMN_EDIT, menuLink('', 'menuAction=htmlUrlType.modify&url_type_id=' . $id), false);

		if ($g_oSec->HasPerm(DCL_ENTITY_URLTYPE, DCL_PERM_DELETE))
			$this->_AddDisplayOption(STR_CMMN_DELETE, menuLink('', 'menuAction=htmlUrlType.delete&url_type_id=' . $id), true);
