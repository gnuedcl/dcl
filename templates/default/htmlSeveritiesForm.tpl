<!-- $Id: htmlSeveritiesForm.tpl,v 1.5 2006/11/27 06:00:52 mdean Exp $ -->
{dcl_validator_init}
...
<form class="styled" method="post" action="{$URL_MAIN_PHP}">
	...
	<fieldset>
		...
		<div class="required">
			<label for="active">{$smarty.const.STR_SEV_ACTIVE}:</label>
			{$CMB_ACTIVE}
		</div>
		<div class="required">
			<label for="short">{$smarty.const.STR_SEV_SHORT}:</label>
		...
		<div class="required">
			<label for="weight">{$smarty.const.STR_SEV_WEIGHT}:</label>
	...
	<fieldset>
		<div class="submit">
			...
			<input type="button" onclick="location.href = '{$URL_MAIN_PHP}?menuAction=boSeverities.showall';" value="{$smarty.const.STR_CMMN_CANCEL}">
		</div>
	</fieldset>
</form>