<html>
	<!-- $Id: htmlOrganizationSelectorControl.tpl,v 1.1.1.1 2006/11/27 05:30:38 mdean Exp $ -->
	<head>
...
{literal}
			...
			{
{/literal}
				var sURL = '{$smarty.const.DCL_WWW_ROOT}main.php?menuAction=htmlOrganizationSelector.showBrowseFrame';
{literal}
...
{literal}
				if (sActiveFilter == "S")
				{
					if (aSelectedID.length > 0)
					{
						...
						if (sID.length > 0)
							sParams += '&filterActive=S&filterID=' + sID;
					}
				}
				else
				{
...
{/literal}
	...
	<body onload="init();">
		<table style="width: 100%;" cellspacing="0">
			<tr>
				<th class="formTitle">Select Organization</th>
				<th class="formLinks">{if $PERM_ADD}<a href="#" onclick="doNew();">{$smarty.const.STR_CMMN_NEW}</a>&nbsp;|&nbsp;{/if}<a href="#" onclick="doSave();">{$smarty.const.STR_CMMN_SAVE}</a>&nbsp;|&nbsp;<a href="#" onclick="doCancel();">{$smarty.const.STR_CMMN_CANCEL}</a></th>
			</tr>
			<tr>
...
{/strip}
							...
							<td style="text-align: right; white-space: nowrap;">
								...
								<br />
								&nbsp;<input type="button" style="width: 54px;" name="btnNav" id="btnNavPrev" value="&lt;&lt;"{$VAL_PREVDISABLED} onclick="prevPage();">
								&nbsp;<input type="button" style="width: 54px;" name="btnNav" id="btnNavNext" value="&gt;&gt;"{$VAL_NEXTDISABLED} onclick="nextPage();">
							</td>
						</tr>
					</table>
				</td>
