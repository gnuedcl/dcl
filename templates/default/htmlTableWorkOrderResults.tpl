...
{section loop=$records name=row}
	...
	{section loop=$records[row] name=item}
		{if !in_array($smarty.section.item.index, $groups) && $smarty.section.item.index < (count($records[row]) + $VAL_ENDOFFSET)}
			...
			{elseif $smarty.section.item.index == $hotlist_ordinal && $records[row][$num_hotlist_ordinal] > 1}{dcl_get_entity_hotlist entity=$smarty.const.DCL_ENTITY_WORKORDER key_id=$records[row][$wo_id_ordinal] key_id2=$records[row][$seq_ordinal] link=Y}
